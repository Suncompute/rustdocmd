...
pub fn write_markdown_and_summary(
    ...
    let tracking_file = target_dir.join(".rustdocmd_generated");
    
    // Lade vorherige Liste generierter Dateien
    let mut previously_generated: Vec<String> = if tracking_file.exists() {
        fs::read_to_string(&tracking_file)?
            .lines()
            .map(|s| s.to_string())
            .collect()
    } else {
    ...
    let mut currently_generated: Vec<String> = Vec::new();
    for block in blocks {
        ...
        currently_generated.push(block.target_md.clone());
    }
    ...
    // 5. Entferne nur Dateien, die vorher von rustdocmd generiert wurden, aber jetzt nicht mehr in blocks sind
    for prev_file in &previously_generated {
        if !currently_generated.contains(prev_file) {
            let file_path = target_dir.join(prev_file);
            if file_path.exists() {
                if dry_run {
                    println!("[dry-run] remove {} (was generated before, now removed from blocks)", file_path.display());
                } else {
                    fs::remove_file(&file_path)?;
                    println!("[cleanup] removed {}", file_path.display());
                }
            }
        }
    }

    // 6. Tracking-Datei aktualisieren
    if !dry_run {
        let tracking_content = currently_generated.join("\n");
